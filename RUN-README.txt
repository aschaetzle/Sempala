...
### Loader (Only Spark)
To execute the loader of Sempala Property Table, is necessary to use the jar file as input of spark-submit.
The necessary parameters for the jar file are:


### Guide to execute Sempala with the Property Table Data Model (Impala version)
To execute the loader of Sempala for Complex Property Table model, the necessary parameters for the jar file that should be set are:
	 l presents the loader of Sempala,
...
To execute the translator of Sempala for Complex Property Table model using Impala, the necessary parameters for the jar file that should be set are:
	...
	-f is the format which in this case is complex_property_table_impala,
	-H is the host where Sempala translator is run,
	-i is the path to the folder of queries,

For example, the parameters of the jar could look like this:
java -jar Sempala.jar t -d ComplexWD100DB -f complex_property_table_impala -H dbisma03.informatik.privat -i ./Q3
