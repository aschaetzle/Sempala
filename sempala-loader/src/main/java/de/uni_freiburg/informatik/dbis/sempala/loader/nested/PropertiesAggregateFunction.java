...
public class PropertiesAggregateFunction extends UserDefinedAggregateFunction {
	...
	public StructType bufferSchema() {
		return new StructType().add("map", DataTypes.createMapType(DataTypes.StringType, 
	...
	private List<String> getStringList(Object rawList){
		return new ArrayList<String>(scala.collection.JavaConverters.asJavaListConverter((
				scala.collection.mutable.WrappedArray.ofRef<String>) rawList ).asJava());
	}
	
	// for each element inside a group, add the new value to the right property in the buffer
	public void update(MutableAggregationBuffer buffer, Row input) {
		...
		HashMap<Object, Object> properties = new HashMap<Object, Object>(scala.collection.JavaConversions.
				asJavaMap(buffer.getMap(0)));
	
		// if the property already exists, append the value at the end of the list
	...
	public void merge(MutableAggregationBuffer buffer1, Row buffer2) {
		...
		while(i < sortedKeys1.length || j < sortedKeys2.length){
			...
			// one of the lists is finished before, add element and skip to next while cycle
			...
			String key1 = sortedKeys1[i]; String key2 = sortedKeys2[j];
			int comparisonKeys = key1.compareTo(key1);

			// the two list for the same key have to be merged (duplicates inside the lists ignored)
			if(comparisonKeys == 0){
