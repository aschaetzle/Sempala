...
CREATE TABLE bigtable_parquet LIKE bigtable_ext STORED AS PARQUET;
...
LOCATION "/user/${username}/${dataset}";

CREATE TABLE triplestore_parquet LIKE triplestore_ext STORED AS PARQUET;
...
def loadAllDatasets(datasetname, usemr, partitioned_table, username):
    ...
    for ds in datasets:
        loadDataset(datasetname, ds, usermr, partitioned_table, username)

def loadDataset(datasetname, dataset, usemr, partitioned_table, username):
    logfile = datasetname+"_loading_impala.log"
    dbname = dataset + "_bigtable_impala"
    impala = ImpalaWrapper(dbname)
    impala.createDB(dbname)
    ...
    if usemr:
        ...
        os.popen('hadoop jar ./SempalaLoader.jar sparql2impala.mapreduce.RDFToCSV.RDFConverter -f 1 ' + pathname + ' 10 ' + dataset)
        # change permissions of created folder
        os.popen('hadoop fs -chmod -R 777 ' + dataset)    
   
    #impala.clearDatabase()
    impala.query(create_triplestore_table_query.substitute(dataset=dataset, username=username))
    ...
    if usemr:
        ...
        if datasetname == 'lubm':
            os.popen('hadoop jar ./SempalaLoader.jar sparql2impala.mapreduce.RDFToCSV.RDFConverter --lubm -f 2 ' + pathname + ' 10 ' + dataset)
        else :
            os.popen('hadoop jar ./SempalaLoader.jar sparql2impala.mapreduce.RDFToCSV.RDFConverter -f 2 ' + pathname + ' 10 ' + dataset)
        os.popen('hadoop fs -chmod -R 777 ' + dataset)    
    ...
    impala.query(query.substitute(dataset=dataset, username=username))
...
def main():
    ...
    try: 
        ...
        username = "user"
        print("Warning! Using default username user. Please specify user for HDFS home folder use.")
    try: 
        datasetname = opts['-dn']
    except KeyError:
        print()
    ...
    try:
        ...
        print("no dataset")
        loadAllDatasets(datasetname, usemr, partitioned_table, username)
    
    # load single dataset
    loadDataset(datasetname, dataset, usemr, partitioned_table, username)
