...
class ImpalaWrapper():
    ...
    def createDB(self, database):
        proc = subprocess.Popen('impala-shell -i dbisma02.informatik.privat -B -q \'CREATE DATABASE IF NOT EXISTS '+database+'; INVALIDATE METADATA \'', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        stdout, stderr = proc.communicate()
        print(stdout)
        print(stderr)
        proc = subprocess.Popen('impala-shell -i dbisma02.informatik.privat -B -q \'INVALIDATE METADATA\'', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
